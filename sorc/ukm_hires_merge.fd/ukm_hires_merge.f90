program ukm_hires_merge
  !-----------------------------------------------------------------
  ! Merges UKM hires data in 8 regional patches into one global array
  ! Data Source: /dcom/us007003/yyyymmdd/wgrbbul/ukmet_hires
  ! Uses bi-linear interpolation to create a 1-deg global dataset.
  ! Fanglin Yang, June 2014
  !-----------------------------------------------------------------

  implicit none

  ! Input data info
  integer, parameter :: np = 8
  integer, parameter :: nx = 108, ny = 162, nxny = nx * ny
  integer, parameter :: mmax = 200000
  real, parameter :: resy = 556.0, resx = 833.0

  logical*1 :: lb(mmax)
  character(len=200) :: input, output, argument
  integer :: nargs, fhr, fhrm6, fhrm12
  integer :: jpds(200), jgds(200), kpds(200), kgds(200)
  integer :: i, j, n, k, ii, jj, iret, kf, nrec, i0

  real :: varp(mmax, np)
  real :: var(nx * 4 + 1, ny * 2)
  real :: xs(np), xe(np), ys(np), ye(np)
  real :: lon(nx * 4 + 1), lat(ny * 2)

  real, parameter :: resg = 1000.0
  integer, parameter :: mx = 360, my = 181, mxmy = mx * my
  real :: varg(mx, my), varg1(mx * my)
  real :: longx(mx), latg(my)
  real :: dy1(my), dy2(my), dx1(mx), dx2(mx)
  integer :: ix(mx), iy(my)
  real :: tmp1, tmp2, sums, sumn

  ! Starting and ending lat and lon of each patch (x1000)
  data xs / 341250, 71250, 161250, 251250, 341250, 71250, 161250, 251250 /
  data xe / 70416, 160416, 250416, 340416, 70416, 160416, 250416, 340416 /
  data ys / 279, 279, 279, 279, -89721, -89721, -89721, -89721 /
  data ye / 89722, 89722, 89722, 89722, -278, -278, -278, -278 /

  ! Compute grid positions
  do n = 1, 4
    i0 = (n - 1) * nx
    do i = i0 + 1, i0 + nx
      lon(i) = xs(n) + resx * (i - i0 - 1)
      if (lon(i) .gt. 360000) lon(i) = lon(i) - 360000
    enddo
  enddo
  lon(4 * nx + 1) = lon(1)

  do j = 1, ny
    lat(j) = ys(5) + resy * (j - 1)
  enddo
  do j = ny + 1, ny * 2
    lat(j) = ys(1) + resy * (j - ny - 1)
  enddo

  do i = 1, mx
    longx(i) = (i - 1) * resg
  enddo
  do j = 1, my
    latg(j) = -90000.0 + (j - 1) * resg
  enddo

  do j = 1, my
    do jj = 1, 2 * ny - 1
      if (latg(j) .ge. lat(jj) .and. latg(j) .lt. lat(jj + 1)) then
        dy1(j) = latg(j) - lat(jj)
        dy2(j) = lat(jj + 1) - latg(j)
        iy(j) = jj
      endif
    enddo
  enddo

  i = 1
  ii = 23
  ix(i) = 23
  dx1(i) = 360000.0 - lon(ii)
  dx2(i) = lon(ii + 1)

  do i = 2, mx
    do ii = 1, 4 * nx
      if (longx(i) .ge. lon(ii) .and. longx(i) .lt. lon(ii + 1)) then
        dx1(i) = longx(i) - lon(ii)
        dx2(i) = lon(ii + 1) - longx(i)
        ix(i) = ii
      endif
    enddo
  enddo

  nargs = command_argument_count()
  if (nargs < 3) then
    write(*, *) 'usage : ukm_hires_merge.x input output fcst_hour'
    stop
  endif

  call get_command_argument(1, input)
  call get_command_argument(2, output)
  call get_command_argument(3, argument)
  read(argument, *) fhr

  print*, "input: ", trim(input)
  print*, "output: ", trim(output)
  print*, "fcst hour:", fhr

  fhrm6 = fhr - 6
  fhrm12 = fhr - 12

  call baopenr(10, trim(input), iret)
  if (iret /= 0) write(6, *) " failed to open ", input
  call baopen(20, trim(output), iret)
  if (iret /= 0) then
    print *, "iret=",iret, "STOP"
    stop
  endif

  nrec = -1
  do while (nrec < 0)
    jpds = -1
    jgds = -1
    do n = 1, np
      jgds(2) = nx
      jgds(3) = ny
      jgds(9) = resx
      jgds(10) = resy
      jgds(4) = ys(n)
      jgds(5) = xs(n)
      jgds(7) = ye(n)
      jgds(8) = xe(n)
      call getgb(10, 0, mmax, nrec, jpds, jgds, kf, k, kpds, kgds, lb, varp(1, n), iret)
      if (kpds(6) /= 7) then
        if (iret /= 0) then
          print *, "iret=",iret, "STOP"
          CALL W3TAGE('ukm_hires_merge ')
          stop
        endif
        do k = 1, 16
          jpds(k) = kpds(k)
        enddo
      endif
      nrec = nrec - 1
    enddo

    if ((jpds(14) == fhr) .or. (jpds(5) == 61 .and. jpds(15) == fhr)) then
      do n = 1, 4
        do j = 1, ny
          do i = 1, nx
            jj = ny + j
            ii = (n - 1) * nx + i
            var(ii, jj) = varp((j - 1) * nx + i, n)
          enddo
        enddo
      enddo
      do n = 5, 8
        do j = 1, ny
          do i = 1, nx
            jj = j
            ii = (n - 5) * nx + i
            var(ii, jj) = varp((j - 1) * nx + i, n)
          enddo
        enddo
      enddo
      do jj = 1, 2 * ny
        var(4 * nx + 1, jj) = var(1, jj)
      enddo

      sums = 0.0
      sumn = 0.0
      do i = 1, 4 * nx
        sums = sums + var(i, 1)
        sumn = sumn + var(i, 2 * ny)
      enddo
      do i = 1, mx
        varg(i, 1) = sums / (4 * nx)
        varg(i, my) = sumn / (4 * nx)
      enddo

      do j = 2, my - 1
        do i = 1, mx
          tmp1 = (var(ix(i), iy(j)) * dy2(j) + var(ix(i), iy(j) + 1) * dy1(j)) / (dy1(j) + dy2(j))
          tmp2 = (var(ix(i) + 1, iy(j)) * dy2(j) + var(ix(i) + 1, iy(j) + 1) * dy1(j)) / (dy1(j) + dy2(j))
          varg(i, j) = (tmp1 * dx2(i) + tmp2 * dx1(i)) / (dx1(i) + dx2(i))
        enddo
      enddo

      do j = 1, my
        do i = 1, mx
          varg1((j - 1) * mx + i) = varg(i, j)
        enddo
      enddo

      kgds(2) = mx
      kgds(3) = my
      kgds(9) = resg
      kgds(10) = resg
      kgds(4) = -90000
      kgds(5) = 0
      kgds(7) = 90000
      kgds(8) = (mx - 1) * resg
      call putgb(20, mxmy, kpds, kgds, lb, varg1, iret)
    endif
  end do

  call baclose(10, iret)
  call baclose(20, iret)
  CALL W3TAGE('ukm_hires_merge ')
end program ukm_hires_merge

